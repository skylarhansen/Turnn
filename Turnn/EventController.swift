...
class EventController {
    ...
    static func createEvent(title: String, location: Location, startTime: NSDate, endTime: NSDate, categories: [Category], eventDescription: String? = "", passwordProtected: Bool? = false, password: String?, contactInfo: String?, image: UIImage?, moreInfo: String?)
    {
    let host = UserController.sharedController.currentUser
        var event = Event(title: title, location: location, startTime: startTime, endTime: endTime, categories: categories, eventDescription: eventDescription, passwordProtected: passwordProtected, password: password, contactInfo: contactInfo, image: image, host: host, moreInfo: moreInfo)
        event.save()
    }
